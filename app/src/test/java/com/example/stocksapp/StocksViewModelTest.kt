...
class StocksViewModelTest {
    ...
    fun testStocks_successScenario() = runTest {
        ...
        testStock("TWTR", "Twitter Inc.", firstStock)
        testStock("APPLE", "Apple Inc.", secondStock)

        //In this way we can test other fields as well by passing to testStock function.
    }

    private fun testStock(ticker: String, name: String, stockItem: StockItem) {
        assertEquals(ticker, stockItem.ticker)
        assertEquals(name, stockItem.name)
    }
